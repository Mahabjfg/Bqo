const axios = require("axios"); const fs = require("fs"); const { createReadStream, unlinkSync } = require("fs-extra");

module.exports = { config: { name: "scloud", category: 'cdna ', author: 'Mostakim x mahabub' },

onReply: async ({ api, event, handleReply }) => { try { const { results, messageID } = handleReply; const choice = event.body - 1;
...
onStart: async ({ api, event, args }) => { try { const input = args.join(" ");
